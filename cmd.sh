...
deployCmd() {
	...
	if [[ $STATUS = "READY"* ]]; then
	    ...
	    RESULT=$(aws gamelift create-fleet --name "Z8_App" --description "Z8 App Server" --build-id $BUILD_ID --ec2-instance-type "t2.medium" --runtime-configuration "GameSessionActivationTimeoutSeconds=600, ServerProcesses=[{LaunchPath=/local/game/App.x86_64, ConcurrentExecutions=10}]" --new-game-session-protection-policy "FullProtection" --ec2-inbound-permissions "FromPort=12350,ToPort=12359,IpRange=0.0.0.0/0,Protocol=UDP" --query "FleetAttributes.Status" --output text);
	    if [[ $RESULT = "NEW"* ]]; then
		    echo "Fleet Created!";
