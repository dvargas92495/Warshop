...
public class ScriptConventionsTests
{
    ...
    {
        List<string> filesWithGetComponent = allFiles.Filter(NoGetComponentHelper);
    ...
    {
        List<string> filesWithSystemDependence = allFiles.Filter(NoSystemDependencyHelper).Filter(f => !f.EndsWith("ZException.cs"));
    ...
    private static List<string> GetAllFiles(string dirPath)
    {
        ...
        List<string> moreFiles = Util.ToList(allDirectories).MapFlattened(GetAllFiles);
